...
## The names of anyone you worked with on this project: Michelle Gee
...
def get_user_tweets(screenname): #anytime you put a twitter handle into this function, JSON + SQL file will be automatically updated 
    ...
    if screenname in CACHE_DICTION:
        ...
        results = api.user_timeline(screen_name = screenname, count = 20) 
        try:
            CACHE_DICTION[screenname] = results
...
for search in all_queries:
	for tweet in CACHE_DICTION[search]:
		...
		cur.execute("INSERT INTO Tweets (tweet_id, tweet_text, user_posted, time_posted, retweets) VALUES (?, ?, ?, ?, ?)",
			(tweet_id, tweet_text, user_posted, time_posted, retweets))

		conn.commit()

for search in all_queries:
	user_id = CACHE_DICTION[search][0]['user']['id_str']
	screen_name = search
	num_favs = CACHE_DICTION[search][0]['user']['favourites_count'] 
	description = CACHE_DICTION[search][0]['user']['description']
...
sqltweets = 'SELECT tweet_id, tweet_text, user_posted, time_posted, retweets FROM Tweets'
sqlusers = 'SELECT user_id, screen_name, num_favs, description FROM Users'
